...
def evaluate_splitter(splitter, documents, question, splitter_name):
    ...
    for i, node in enumerate(raw_nodes, 1):
        ...
        if isinstance(splitter, SentenceWindowNodeParser):
            ...
            # 拿到这个句子前后的“窗口上下文” (比如前后各多带一句话)
            window_context = node.metadata.get("window", "N/A - Window content missing")
            print(f"   核心内容: \"{original_text}\"")
            print(f"   完整窗口上下文: \"{window_context}\"")
        else:
            # 对于普通的切块算法 (比如按字数切)，直接打印切出来的文本
    ...
    if isinstance(splitter, SentenceWindowNodeParser):
        ...
        query_engine_params["node_postprocessors"] = [postprocessor]
...
def chinese_sentence_splitter(text: str) -> list[str]:
    """
    自定义的中文句子切分器。
    按中文或英文的句号、问号、感叹号，以及换行符进行切分，并保留标点符号。
    """
    # 使用正则表达式，在句号、问号、感叹号或换行符后进行切分
    # (?<=[。！？!?\n]) 意思是：如果前面是这些符号，就在这里切一刀
    sentences = re.split(r'(?<=[。！？!?\n])', text)
    
    # 清理空白字符并过滤掉空字符串
    return [s.strip() for s in sentences if s.strip()]

# 初始化 Sentence Window 切片器，并强行替换它的默认切分逻辑
sentence_window_splitter = SentenceWindowNodeParser(
    window_size=1, # 窗口大小：前后各带1句话
    window_metadata_key="window",
    original_text_metadata_key="original_text",
    sentence_splitter=chinese_sentence_splitter # <--- 核心修复：注入中文切分规则
)
