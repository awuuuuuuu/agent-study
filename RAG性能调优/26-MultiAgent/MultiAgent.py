...
# 加载环境变量：find_dotenv() 会向上查找父目录中的 .env 文件
...
class TaskNode:
    ...
    # [优化1] retry_count 移到 TaskNode，确保重试计数跨调用持久化
    retry_count: int = 0
...
class SystemState(TypedDict):
    ...
    current_phase: str
    error_count: int
    monitoring_metrics: Dict[str, Any]  # 最终监控汇总数据
...
class BaseToolAgent:
    ...
    def execute(self, params: Dict[str, Any], timeout: float = 30.0) -> ToolExecutionResult:
        """[优化3] 新增 timeout 参数，通过 ThreadPoolExecutor 实现真正的超时控制"""
...
class NewsAgent(BaseToolAgent):
    ...
    def _execute_core(self, params: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # results 是 List[dict]，每项含 content/url
...
class TopAgent:
    ...
    def plan_tasks(self, user_question: str) -> List[TaskNode]:
        """智能任务规划 - 生成 DAG 结构的任务图"""
        planning_prompt = f"""
        作为投资分析系统的Top Agent，请为用户问题制定详细的执行计划：
        问题：{user_question}

        可用工具及其特性：
        1. stock_price - 获取股票价格数据（高优先级，关键任务）
        2. news - 获取相关新闻（中优先级，可降级）
        3. sentiment - 情感分析（低优先级，依赖news）
        ...
        try:
            # [优化4] 用正则提取 JSON，兼容 LLM 在输出中附加 ```json``` 的情况
            text = response.text
            ...
            for task_data in plan_data["tasks"]:
                ...
                tasks.append(task)
            self._print_task_dag(tasks)
            return tasks
        except Exception as e:
            print(f"[WARN] LLM 规划解析失败（{e}），使用默认计划")
            tasks = self._get_default_plan()
            self._print_task_dag(tasks)
            return tasks

    def _print_task_dag(self, tasks: List[TaskNode]):
        print("\n=== 任务规划 DAG ===")
        for task in tasks:
            deps = " → ".join(task.dependencies) if task.dependencies else "无"
            critical = "【关键】" if task.is_critical else "      "
            print(f"  {critical} [{task.task_id}] {task.tool_name}"
                  f"  优先级:{task.priority}  超时:{task.timeout}s"
                  f"  最大重试:{task.max_retries}  依赖:{deps}"
                  f"  参数:{task.params}")
        print("=" * 40)
    ...
    def execute_task(self, task: TaskNode, state: SystemState) -> ToolExecutionResult:
        ...
        # [优化3] 将 TaskNode.timeout 传入 execute，实现真实超时控制
    ...
    def should_retry_task(self, task: TaskNode, result: ToolExecutionResult) -> bool:
        """[优化1] 重试判断基于 TaskNode.retry_count，而非 ToolExecutionResult"""
...
def build_workflow_nodes(agent: TopAgent):
    """用闭包注入单例 TopAgent，避免每个节点重建实例"""

    def initialize_system(state: SystemState):
        """系统初始化：规划任务 DAG"""
        task_dag = agent.plan_tasks(state["user_question"])
        return {
            "task_dag": task_dag,
    ...
    def execute_next_task(state: SystemState):
        """执行下一个任务节点"""
        if agent.check_early_exit(state):
            return {
        ...
        next_task = agent.dynamic_dispatch(state)
        if not next_task:
            return {
                "should_continue": False,
        ...
        result = agent.execute_task(next_task, state)
        ...
        if result.status == TaskStatus.COMPLETED:
            ...
            if agent.should_retry_task(next_task, result):
    ...
    def generate_final_report(state: SystemState):
        """生成最终投资报告"""
        recommendation = agent.generate_final_recommendation(state)
        dashboard_summary = agent.monitoring_dashboard.get_dashboard_summary()
        return {
    ...
    return initialize_system, execute_next_task, generate_final_report
...
top_agent = TopAgent()
initialize_system, execute_next_task, generate_final_report = build_workflow_nodes(top_agent)
